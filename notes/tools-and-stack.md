...
| **pgx/v5** | v5.10.0 | `go get github.com/jackc/pgx/v5` | Postgres driver (ผ่าน stdlib → `sql.Open("pgx",...)`) |
...
| **go.uber.org/mock** | `go install go.uber.org/mock/mockgen@latest` | gen mock ของ db layer |
...
- `sqlc.yaml` — config sqlc (มี override `users.password` → `json:"-"`)
